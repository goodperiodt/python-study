...
- 문자열의 format 함수를 사용하면 서식 문자를 지정하는 것 보다(%)
...
# 소수점 표현
pi = 3.14592
print('원주율은 {:0.2f}입니다.'.format(pi)) # 반올림까지 처리

# f 문자열 포맷팅
# 파이썬 3.6버전 이후로 사용할 수 있는 기능입니다.
# 접두어 f를 문자열 앞에 붙여서 사용합니다.

print(f'{month}월 {day}일은 {anni}입니다.') #자바스크립트의 문자열 리털과 비슷한 느낌

# {} 안에서는 수식도 사용이 가능합니다.
salary = 300
print(f'보너스를 합한 월급: {salary + 100}') # 수식도 사용이 가능하다
